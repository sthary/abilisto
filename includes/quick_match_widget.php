...
// worker/includes/quick_match_widget.php
    ...
    $stmt_activeMatches = $conn->prepare("
        SELECT qb.*, qs.*, u.full_name as client_name,
               FLOOR(EXTRACT(EPOCH FROM (qs.expires_at - NOW())) / 60) as minutes_left
    ...
    ");
    $stmt_activeMatches->execute([$worker_id]);
    $activeMatches_rows = $stmt_activeMatches->fetchAll();

    if (count($activeMatches_rows) > 0):
    ?>
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px; padding: 1rem; margin-bottom: 1rem; color: white;">
        ...
        <?php foreach ($activeMatches_rows as $match):
