...
$stmt = $conn->prepare("INSERT INTO messages (booking_id, sender_id, message, message_type) VALUES (?, ?, ?, ?) RETURNING id");

if ($stmt->execute([$booking_id, $sender_id, $message, $message_type])) {
    echo json_encode(['success' => true, 'message_id' => $stmt->fetchColumn()]);
} else {
