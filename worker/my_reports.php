...
// Fetch all reports from this worker
$sql = "SELECT r.*, 
               u.full_name as reported_user_name,
               u.role as reported_user_role,
               u.profile_pic as user_profile,
...
function getReasonLabel($reason) {
    $reasons = [
        ...
        'cancelled_last_minute' => 'Client cancelled at last minute',
        'unprofessional' => 'Unprofessional behaviour / harassment',
        'non_payment' => 'Client refused to pay',
        'false_accusation' => 'False accusation against me',
        'abusive_language' => 'Abusive language / threats',
        'unsafe_environment' => 'Unsafe working environment',
        'misleading_info' => 'Client provided misleading information',
...
function getRoleBadge($role) {
    $badges = [
        'client' => ['bg' => 'bg-green-100 dark:bg-green-900/20', 'text' => 'text-green-700 dark:text-green-400', 'icon' => 'person'],
        'worker' => ['bg' => 'bg-blue-100 dark:bg-blue-900/20', 'text' => 'text-blue-700 dark:text-blue-400', 'icon' => 'handyman'],
        'business' => ['bg' => 'bg-purple-100 dark:bg-purple-900/20', 'text' => 'text-purple-700 dark:text-purple-400', 'icon' => 'business']
    ];
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        ...
        <div class="flex gap-3">
            <a href="report_user.php" class="flex items-center gap-2 px-5 py-2.5 bg-primary hover:bg-blue-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-primary/20">
            ...
            <div class="report-card bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <div class="p-6 border-b border-slate-100 dark:border-slate-700">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <!-- User Avatar -->
                            <?php if (!empty($report['user_profile']) && file_exists("../uploads/profiles/" . $report['user_profile'])): ?>
                            <img src="../uploads/profiles/<?php echo $report['user_profile']; ?>" 
                            ...
                            <div class="w-14 h-14 rounded-full bg-gradient-to-br from-primary to-indigo-600 flex items-center justify-center text-white font-bold text-xl">
                                <?php echo strtoupper(substr($report['reported_user_name'], 0, 2)); ?>
                            </div>
                            <?php endif; ?>
                            ...
                            <div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">
                                        Report against <?php echo htmlspecialchars($report['reported_user_name']); ?>
                                    </h3>
                                    <span class="px-2 py-0.5 rounded-full text-xs font-medium <?php echo $role_badge['bg']; ?> <?php echo $role_badge['text']; ?> flex items-center gap-1">
                                        <span class="material-symbols-rounded text-[12px]"><?php echo $role_badge['icon']; ?></span>
                                        <?php echo ucfirst($report['reported_user_role']); ?>
                                    </span>
                                </div>
                ...
                <div class="px-6 py-3 bg-slate-50 dark:bg-slate-700/30 border-t border-slate-100 dark:border-slate-700">
                    <div class="flex items-center justify-between text-xs text-slate-400">
                        ...
                        <span>Case Reference: WRPT-<?php echo str_pad($report['id'], 6, '0', STR_PAD_LEFT); ?></span>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
        ...
        <?php else: ?>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-3xl p-16 text-center border border-slate-200 dark:border-slate-700">
                ...
                <p class="text-slate-500 dark:text-slate-400 mb-8 max-w-md mx-auto">
                    You haven't submitted any reports. If you encounter an issue with a client, you can report them from the report page.
                </p>
                <a href="report_user.php" class="inline-flex items-center gap-2 bg-primary hover:bg-blue-700 text-white font-bold py-4 px-8 rounded-2xl transition-all shadow-lg shadow-primary/20">
                    <span class="material-symbols-rounded">add</span>
                    Create a Report
                </a>
            </div>
    ...
    <div class="mt-12 bg-gradient-to-r from-slate-50 to-white dark:from-slate-800 dark:to-slate-800 rounded-3xl p-8 border border-slate-200 dark:border-slate-700">
        <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-6">Understanding Report Status</h3>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="flex items-start gap-3">
                <div class="w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-900/20 flex items-center justify-center shrink-0">
                    <span class="material-symbols-rounded text-amber-600 dark:text-amber-400 text-sm">pending</span>
                </div>
                <div>
                    <h4 class="font-bold text-sm mb-1 text-slate-900 dark:text-white">Pending Review</h4>
                    <p class="text-xs text-slate-500">Your report is in queue for admin review</p>
                </div>
            </div>
            <div class="flex items-start gap-3">
                <div class="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/20 flex items-center justify-center shrink-0">
                    <span class="material-symbols-rounded text-blue-600 dark:text-blue-400 text-sm">visibility</span>
                </div>
                <div>
                    <h4 class="font-bold text-sm mb-1 text-slate-900 dark:text-white">Under Review</h4>
                    <p class="text-xs text-slate-500">Admin is actively investigating your report</p>
                </div>
            </div>
            <div class="flex items-start gap-3">
                <div class="w-8 h-8 rounded-full bg-green-100 dark:bg-green-900/20 flex items-center justify-center shrink-0">
                    <span class="material-symbols-rounded text-green-600 dark:text-green-400 text-sm">check_circle</span>
                </div>
                <div>
                    <h4 class="font-bold text-sm mb-1 text-slate-900 dark:text-white">Resolved</h4>
                    <p class="text-xs text-slate-500">Issue has been addressed and closed</p>
                </div>
            </div>
            <div class="flex items-start gap-3">
                <div class="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center shrink-0">
                    <span class="material-symbols-rounded text-slate-600 dark:text-slate-400 text-sm">do_not_disturb</span>
                </div>
                <div>
                    <h4 class="font-bold text-sm mb-1 text-slate-900 dark:text-white">Dismissed</h4>
                    <p class="text-xs text-slate-500">Report was cancelled or not substantiated</p>
                </div>
            </div>
        </div>
    </div>
