...
");
$checkStmt->bind_param("ii", $broadcast_id, $worker_id);
$checkStmt->execute();
...
try {
    ...
    ");
    $acceptedCheck->bind_param("i", $session_id);
    $acceptedCheck->execute();
    $acceptedResult = $acceptedCheck->get_result();
    $accepted = $acceptedResult->fetch_assoc();
    ...
    $updateStmt = $conn->prepare("
        ...
        WHERE id = ?
    ");
    $updateStmt->bind_param("si", $status, $broadcast_id);
    $updateStmt->execute();
    ...
    if ($action === 'accept') {
        ...
        ");
        $updateSession->bind_param("i", $session_id);
        $updateSession->execute();
        
        // Get worker name
        $workerInfo = $conn->query("SELECT full_name FROM users WHERE id = $worker_id")->fetch_assoc();
        ...
        ");
        $expireStmt->bind_param("ii", $session_id, $broadcast_id);
        $expireStmt->execute();
        ...
        $statsStmt = $conn->prepare("
            ...
            WHERE session_id = ?
        ");
        $statsStmt->bind_param("i", $session_id);
        $statsStmt->execute();
