...
if (isset($_GET['action']) && isset($_GET['doc_id']) && isset($_GET['worker_id'])) {
    ...
    if ($action == 'approve') {
        ...
        $stmt = $conn->prepare("UPDATE worker_docs SET status='Verified' WHERE id=?");
        $stmt->execute([$doc_id]);

        // B. Enable 'TESDA Verified' badge in worker profile
        // Only if the doc is a TESDA Cert (NC I, II, or III)
        $dt_stmt = $conn->prepare("SELECT doc_type FROM worker_docs WHERE id=?");
        $dt_stmt->execute([$doc_id]);
        $doc_type = $dt_stmt->fetch()['doc_type'];

        if (strpos($doc_type, 'NC') !== false) {
            $stmt = $conn->prepare("UPDATE worker_profiles SET is_tesda_verified=TRUE WHERE user_id=?");
        ...
        // Reject
        $stmt = $conn->prepare("UPDATE worker_docs SET status='Rejected' WHERE id=?");
        $stmt->execute([$doc_id]);
